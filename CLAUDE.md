# Class Art Wall — how to work in this repo

This is a shared class wall. Everyone has their **own** slot: `pieces/<their-name>/art.html`.
...
2. **When the user asks to save / share / publish / "put it on the wall,"** run this exact
   ...
   git pull --rebase --autostash            # brings in others' work first (no conflicts — separate files)
   git push
   ```

3. **If `git push` is ever rejected,** just run `git pull --rebase --autostash` then `git push`
   again, and repeat until it succeeds. It will always succeed, because every person edits a
